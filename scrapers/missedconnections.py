# requres python 2.7
# pip install requests lxml cssselect unicodecsv
...
for i in range(0, 700+100, 100):
	...
	for row in doc.cssselect('.row'):
		...
		missed_connections.append((pid,date,text))
		print pid + '\t' + date + '\t' + text

	# raw_input("Press Enter to continue...")

with open("missedconnections.csv", "w") as outfile:
	writer = csv.writer(outfile)
	writer.writerow(["pid","date","text"])
	writer.writerows(missed_connections)